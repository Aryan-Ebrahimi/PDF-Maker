...
# Bot watermark text
WATERMARK_TEXT = "Generated by Nova PDF Maker Bot (@33333)"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    keyboard = [
        [InlineKeyboardButton("📸 Convert Photos to PDF", callback_data="photo_to_pdf")],
        [InlineKeyboardButton("📝 Convert Text to PDF", callback_data="text_to_pdf")],
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    await update.message.reply_text(
        "✨ Welcome to the Nova File Converter Bot! ✨\n"
        "Choose an option:",
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if query.data == "photo_to_pdf":
        await query.edit_message_text(
            "📸 Send the photos you want to convert to PDF.\n"
            "When done, type 'done' ✅"
        )
        return PHOTO_TO_PDF
...
async def check_admin_password(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if message_text == ADMIN_PASSWORD:
        ...
        keyboard = [
            [InlineKeyboardButton("👥 Show User Stats", callback_data="show_users")],
            [InlineKeyboardButton("📂 Get User PDFs", callback_data="get_pdfs")],
        ...
        if context.user_data["failed_attempts"] >= 3:
            await update.message.reply_text("❌ Too many failed attempts. Access blocked!")
            context.user_data["failed_attempts"] = 0
            return ConversationHandler.END
        await update.message.reply_text(
            f"❌ Invalid input. {3 - context.user_data['failed_attempts']} attempts left."
        )
        return START
...
async def photo_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if update.message.text and update.message.text.lower() == "done":
        if not context.user_data["photos"]:
            await update.message.reply_text("⚠️ No photos sent! Try again.")
            return PHOTO_TO_PDF
        
        context.user_data["conversion_type"] = "photo"
        await update.message.reply_text("📄 What would you like to name your PDF file?")
        return ASK_FILENAME

    if not update.message.photo:
        await update.message.reply_text("📸 Please send only photos or type 'done'.")
        return PHOTO_TO_PDF
    ...
    await update.message.reply_text(f"✅ Photo {len(context.user_data['photos'])} received.\nNext photo or 'done'?")
    return PHOTO_TO_PDF
...
async def ask_filename_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not filename:
        await update.message.reply_text("⚠️ Please enter a valid filename!")
        return ASK_FILENAME
    ...
    if conversion_type == "photo":
        await update.message.reply_text(f"⏳ Converting photos to PDF as '{filename}'...")
        ...
        try:
            ...
            try:
                with open(pdf_path, "rb") as f:
                    await update.message.reply_document(f, filename=filename, caption="📄 PDF file is ready!")
                ...
                await update.message.reply_text("⚠️ File upload failed. Please try again.")
                return PHOTO_TO_PDF
        ...
        await update.message.reply_text(f"⏳ Converting text to PDF as '{filename}'...")
        pdf = FPDF()
        pdf.add_page()
        ...
        try:
            ...
            await update.message.reply_text(f"⚠️ Error generating PDF: {str(e)}")
            return TEXT_TO_PDF
    ...
    keyboard = [
        [InlineKeyboardButton("📸 Convert New Photos", callback_data="photo_to_pdf")],
        [InlineKeyboardButton("📝 Convert New Text", callback_data="text_to_pdf")],
    ]
    await update.message.reply_text("✅ PDF created successfully! What’s next?", reply_markup=InlineKeyboardMarkup(keyboard))
    return START
...
async def admin_button(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    return_keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back to Admin Panel", callback_data="back_to_admin")]])
    ...
    if query.data == "show_users":
        ...
        if not users:
            await query.edit_message_text("⚠️ No users found!")
        else:
            response = "👥 User Stats:\n\n"
            for user_id, username, pdf_count in users:
                response += f"Username: @{username}\nID: {user_id}\nPDFs Created: {pdf_count}\n\n"
            await query.edit_message_text(response)
        await query.message.reply_text("🔧 What’s next?", reply_markup=return_keyboard)
        return ADMIN

    elif query.data == "get_pdfs":
        await query.edit_message_text("📂 Please send the User ID to get their PDFs:")
        return ADMIN_GET_PDFS

    elif query.data == "clear_data":
        keyboard = [
            [InlineKeyboardButton("✅ Confirm", callback_data="confirm_clear")],
            [InlineKeyboardButton("❌ Cancel", callback_data="back_to_admin")],
        ]
        await query.edit_message_text(
            "⚠️ Are you sure you want to clear all user data and PDFs? This action cannot be undone!",
            reply_markup=InlineKeyboardMarkup(keyboard)
        )
        ...
        try:
            clear_all_data()
            await query.edit_message_text("🗑️ All user data and PDFs have been cleared successfully!")
        except Exception as e:
            await query.edit_message_text(f"⚠️ Error clearing data: {str(e)}")
        await query.message.reply_text("🔧 What’s next?", reply_markup=return_keyboard)
        return ADMIN
...
async def admin_get_pdfs_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        await update.message.reply_text("⚠️ Please enter a valid User ID (numeric)!")
        return ADMIN_GET_PDFS
    ...
    if not pdfs:
        await update.message.reply_text(f"⚠️ No PDFs found for User ID {user_id}!", reply_markup=return_keyboard)
    else:
        await update.message.reply_text(f"📂 Sending {len(pdfs)} PDFs for User ID {user_id}...")
        for pdf_path, in pdfs:
            if os.path.exists(pdf_path):
                with open(pdf_path, "rb") as f:
                    await update.message.reply_document(f, caption=f"PDF from User ID {user_id}")
            else:
                await update.message.reply_text(f"⚠️ File {pdf_path} not found on server!")
        await update.message.reply_text("🔧 What’s next?", reply_markup=return_keyboard)

    return ADMIN

# Cancel function
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    await update.message.reply_text("❌ Operation canceled. Come back anytime!")
    return ConversationHandler.END
